    $(document).ready(function() {
        ...
        function updatePinnedComm(pinnedCommList) {
            ...
            $.each(pinnedCommList, function(index, pinnedComm) {
                var row = $("<tr>").addClass("clickable").click(function() {
                    window.location.href = "/JoinTree/community/freeCommList/freeCommOne?boardNo=" + pinnedComm.boardNo;
                });
                row.append("<td width='10%'>" + pinnedComm.boardNo + "</td>");
                var titleCell = $("<td width='40%'>");
                if (pinnedComm.commentCnt > 0) {
                    titleCell.append("<span class='selected-title' style='color:red;'>[공지] " + pinnedComm.boardTitle + " [" + pinnedComm.commentCnt + "]</span>");
                } else {
                    titleCell.append("<span class='selected-title' style='color:red;'>[공지] " + pinnedComm.boardTitle + "</span>");
                }
        ...
        function updatePage(data) {
            ...
            $.each(data.commList, function(index, comm) {
                var row = $("<tr>").addClass("clickable").click(function() {
                    window.location.href = "/JoinTree/community/freeCommList/freeCommOne?boardNo=" + comm.boardNo;
                });
                row.append("<td width='10%'>" + comm.boardNo + "</td>");
                var titleCell = $("<td width='40%'>");
                if (comm.commentCnt > 0) {
                    titleCell.append("<span class='selected-title'>" + comm.boardTitle + " [" + comm.commentCnt + "]</span>");
                } else {
                    titleCell.append("<span class='selected-title'>" + comm.boardTitle + "</span>");
                }
                row.append(titleCell);
